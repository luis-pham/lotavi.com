...
export default function StaffPage() {
  ...
  const login = async (event: React.FormEvent) => {
    ...
    try { await api("/api/v1/auth/login", { method: "POST", json: { email, password } }); localStorage.setItem("lotiva-staff-authed", "1"); router.push("/staff/my-work"); }
    catch (err) { setError((err as Error).message); } finally { setBusy(false); }
  };
  return <main className="console-login"><form className="console-login-card" onSubmit={login}>
    <div className="console-brand-mark">L</div><h1>Staff workspace</h1><p>Sign in to start your shift.</p>
